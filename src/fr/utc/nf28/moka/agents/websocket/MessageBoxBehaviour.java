		...
		if (message == null) {
			...
			try {
				...
				if (JadeUtils.TRANSACTION_TYPE_CONNECTION.equals(transactionType)) {
					...
					request = WebSocketRequestFactory.createAddItemRequest(item.getType(), item.getId(), item.getContentEntries(), item.getX(),
							item.getY(), item.getWidth(), item.getHeight(), item.getRotateX(), item.getRotateY(), item.getRotateZ());
				...
				} else if (JadeUtils.TRANSACTION_TYPE_LOGOUT.equals(transactionType)) {
					final String ip = (String) transaction.getContent();
					request = WebSocketRequestFactory.createRemoveUserRequest(ip);
				}
